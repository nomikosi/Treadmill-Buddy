...
public enum CalorieAlgorithm {
    ACSM_FLAT(
            "ACSM flat treadmill",
            "Most commonly used in exercise-science references. Uses ACSM walking/running oxygen-cost equations at 0% incline."
    ) {
        @Override
        public double kcalPerMinute(UserProfile profile, double speedKmh) {
    ...
    COMPENDIUM_MET_GROSS(
            ...
            "Uses speed bands from the Compendium of Physical Activities and includes resting energy."
    ) {
        @Override
        public double kcalPerMinute(UserProfile profile, double speedKmh) {
    ...
    COMPENDIUM_MET_ACTIVE(
            ...
            "Uses Compendium MET speed bands minus 1 MET, a conservative active-calorie estimate."
    ) {
        @Override
        public double kcalPerMinute(UserProfile profile, double speedKmh) {
    ...
    DISTANCE_COST(
            ...
            "Uses common cost-of-transport estimates: about 0.8 kcal/kg/km walking, 1.0 running."
    ) {
        @Override
        public double kcalPerMinute(UserProfile profile, double speedKmh) {
    ...
    public abstract double kcalPerMinute(UserProfile profile, double speedKmh);
