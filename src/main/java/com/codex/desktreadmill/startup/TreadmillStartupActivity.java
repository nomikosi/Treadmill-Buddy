...
public final class TreadmillStartupActivity implements ProjectActivity {
    ...
    public @Nullable Object execute(@NotNull Project project, @NotNull Continuation<? super Unit> continuation) {
        if (!TreadmillSettings.getInstance().getProfile().isComplete()) {
            // withAction, so the balloon steps aside like every other one but
            // the invitation stays in the Notifications tool window - the
            // profile is still incomplete until it's actually filled in.
